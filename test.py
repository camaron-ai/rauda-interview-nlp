...
def create_fake_rate_limit_error() -> RateLimitError:
    ...
    fake_rate_limit_error = RateLimitError(
        "This is a fake exception", response=fake_response, body=None
    )
...
class TestTicketEvaluator:
    ...
    def test_generate_evaluation_with_failure(self, client: OpenAI, max_tries: int):
        """
        ...
        with patch.object(
            ...
            "parse",
            side_effect=fake_rate_limit_error,
        ) as mock_api_call, patch("time.sleep") as sleep_mock:

            # make sure it raise an Error
            with pytest.raises(
                Exception, match="Max retries exceeded. Try again later."
            ):
