...
class TicketEvaluator:
    ...
    def __init__(
        self, client: OpenAI, system_prompt: str, model_name: str, max_tries: int = 5
    ):
    ...
    def generate_evaluation(self, request: TicketEvalRequest) -> TicketEvalResponse:
        """
        ...
        while tries < self.max_tries:
            try:
                ...
                if not (1 <= ticket_evaluation.content_score <= 5):
                    raise ValueError(
                        f"content_score should be between 1 and 5, got: {ticket_evaluation.content_score}"
                    )

                if not (1 <= ticket_evaluation.format_score <= 5):
                    raise ValueError(
                        f"content_score should be between 1 and 5, got: {ticket_evaluation.format_score}"
                    )
...
def read_tickets(tickets_path: str) -> List[TicketEvalRequest]:
    ...
    tickets = [
        ...
        for i, (ticket_str, reply_str) in enumerate(
            zip(pd_tickets["ticket"], pd_tickets["reply"])
        )
    ]
    return tickets
...
def evaluate_tickets_usecase(
    tickets: List[TicketEvalRequest], evaluator: TicketEvaluator, output_csv_path: str
):
    output = []
...
if __name__ == "__main__":
    ...
    ), "Please provide an OPENAI_API_KEY in your .env file"

    client = OpenAI()
    evaluator = TicketEvaluator(
        client=client, system_prompt=SYSTEM_PROMTP, model_name="gpt-4o-2024-08-06"
    )
